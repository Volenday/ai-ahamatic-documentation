...
| 4 — Client surface shape | Moderate | **PWA-only vs. PWA + one cross-platform app vs. cross-platform everywhere vs. native per platform**, containing the React-Native-vs-Flutter question. Bears on **C-20**. | ✅ **Decided** — ADR-007: platform offers **both a web form and a genuine mobile artifact**, builder-selectable (PWA-only fails C-20's store-publishing and device-capability requirements); **mobile runtime is Flutter**, reversing §5/ADR-001's React Native retention on criteria 7/9/10 plus structural web-mobile parity verifiability. Server and web layers unchanged. |
