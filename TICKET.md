...
| H9 | Invariant Enforcement Design — Each of INV-01–INV-09 Realized as a Verifiable Blocking Check (Layer 2, first document of that layer). **Verified 2026-08-03:** all nine invariants covered; §8 boundaries hold (tenant-isolation named "sharpest boundary" explicitly); no ADR recorded — the map's mechanism-reversal grading confirmed, not overridden; **ADR-018 remains unconsumed** for the next document with a genuine decision | ✅ Done | `docs/design/invariant-enforcement-design.md` |
| H9a | V1.0 Sizing Amendment (D-26) — ~100 applications total, lead-confirmed, replacing the ≤50/10×5 team assumption in `platform-data-model-design.md` §11/§14 and `technology-stack-design.md` §18.6. **Verified 2026-08-03:** independent arithmetic check (not merely cited) confirmed "theoretical, not pressing" still holds at ~100; third stale occurrence (§14) found by the Executor's own sweep, not named in the ticket; **one residual inconsistency caught afterward** — the registry-indirection headroom paragraph still read "from under 50" two paragraphs after §11's ~100, corrected post-handoff | ✅ Done | `docs/design/platform-data-model-design.md`, `docs/design/technology-stack-design.md` |
...
> **Corrected 2026-08-03 — this was NOT a map defect; my earlier note here was wrong and is superseded (`BACKLOG.md` §1c).** `implementation-document-map.md` already discloses and justifies `ai-assisted-builder-tooling-design.md`'s dependency on three Layer 6 documents (`agent-runtime-and-control-design.md`, `human-in-the-loop-design.md`, `prompt-and-context-assembly-design.md`) in its own "Design layers and dependency ordering" section, under **"One cross-layer exception"** — naming it, explaining why (C-19 inherits the Meta-Operations guardrails per `01-business-and-ux/02-prd.md` §4), and stating it is the *only* such case. An exhaustive topological sort of all 39 unwritten documents independently confirmed that last claim. **What the sort still earns its place for:** confirming, rather than assuming, that this is the only exception, and placing the document correctly at **H45** — after its three Layer 6 prerequisites — which is what the map's own exception note implies once read together with the per-row dependencies.

| # | Ticket Title | Output File | Depends On (design docs) | Status |
|---|---|---|---|---|
| H10 | Tenant Isolation and Access Control Design. **Verified 2026-08-03:** all 13 sections present; five actor classes genuinely covered; RLS-reconsideration finding (§9) independently engages the isolation-strength question rather than repeating ADR-004's portability rationale, and its MySQL/MariaDB citation checked against `technology-stack-design.md` §14.5; no new tables invented (`application_metadata` confirmed pre-existing); **ADR-018 recorded** — the one genuine decision, with question + criteria as first-class fields per the extended convention | ✅ Done | `docs/design/tenant-isolation-and-access-control-design.md` |
| H11 | Authentication and Identity Design. **Verified 2026-08-03:** Authenticated Actor Identity object cross-checked field-by-field against tenant-isolation's actual Registry Accessor precondition and Context Resolution Point step 3 — no overreach, no missing field; Authentication Gate placement verified against `architecture-realization-design.md`'s literal component text, not assumed; **ADR-019 recorded** — full question + criteria-applied fields, honest non-discriminating criterion retained rather than dropped | ✅ Done | `docs/design/authentication-and-identity-design.md` |
| H12 | ⚠ **Corrected 2026-08-03 — this row originally duplicated H10.** The ticket actually executed under the name "H10" was tenant-isolation (this row's original document), not the scalability document the durable schedule had assigned to H10. Git history is authoritative (`PROCESS.md` §3): H10 stays tenant-isolation, permanently. **This row is repointed to the document that fell out of the schedule as a result — Scalability, Availability, and Performance Design** | `docs/design/scalability-availability-and-performance-design.md` | `architecture-realization-design.md`; `technology-stack-design.md` | 🟢 Ready — no unwritten prerequisite |
...
> **⚠ `invariant-enforcement-design.md`'s §4.1 citations are fragile — verify before extending.** H9's self-review caught two citation errors before finalizing: INV-05 and INV-06's structural-half rows were initially swapped against `architecture-realization-design.md` §4.1 (row 3 is INV-06's; row 4 is INV-05's, per that document's own §4.4), and a quote blending two adjacent subsections was split and misattributed. Both were fixed, but the pattern — two invariants citing adjacent numbered rows in another document — is exactly the shape that produces a silent swap on the next edit. **Any future insertion into this document must re-verify both mappings**, not just run the usual §N sweep.
