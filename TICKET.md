...
| H12 | Scalability, Availability, and Performance Design. **Verified 2026-08-03:** connection-pooling mechanism (SET ROLE per-checkout) decouples pool size from schema count, correctly reframing the crossover question; crossover finding (~500/~2,000 schemas) explicitly flagged as reasoned engineering waypoint, not verified benchmark; synchronous-replication no-ADR call checked — genuine forced consequence of NFR §7's zero-data-loss floor, not a dodge; **ADR-020 recorded** with full question + criteria fields | ✅ Done | `docs/design/scalability-availability-and-performance-design.md` |
