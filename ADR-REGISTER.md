...
A navigational index of every design-phase ADR: its identifier, status, owning document, cost to reverse, and the upstream decisions it assumes. **Last updated 2026-08-11** (post-H41 — ADR-052 registered at its own ticket's close, **opening Layer 6**; ADR-051 at H40's, closing Layer 5; ADR-050 at H39's; ADR-049 at H38's; ADR-048 at H37's; ADR-047 at H36's; ADR-046 at H35's, opening Layer 5. ADR-045 registered at H34's close, closing Layer 4; its row was reconciled at that close against the owning document, having been pre-written from a superseded draft of the same ticket that was set aside before H34 ran; ADR-017 discharged from Owed to Approved at H31; ADR-038 was the first under `PROCESS.md` §3's new rule. Prior entry: post-H26c sync, ADR-024 through ADR-037 added. The register had drifted **fourteen** ADRs behind in a single day, having been synced at H15 the day before. See live issue 8.)
...
**Thirty-two ADRs now await lead approval — ADR-021 through ADR-052.** The earlier claim here — that every ADR was closed and nothing awaited approval — was true of the Layer 1 set at H6 and has not been true of the phase since H13. The three ADRs that waited on the data model (001, 008, 009) remain discharged by H6 on the evidence of `02-platform-data-model-design.md`.
...
| 🔶 **Provisional — Pending Lead Approval** | **32** | **ADR-021**–**ADR-052** *(H13 through H41)* — every ADR produced since H13, none in `DECISIONS.md` |
...
**Thirty-two ADRs now await lead approval — ADR-021 through ADR-052, none recorded in `DECISIONS.md`.** That is every ADR produced since H13, and it is the design phase's largest standing decision backlog. See live issue 8 on why the register keeps falling behind.
...
| **046** | Structurally separate tier instantiation; deploy-time-injected tier/region identity; discharge of the four named provisioning obligations inside ADR-010's portable subset | `01-environment-and-configuration-design.md` §12.1 | 🔶 **Provisional** (H35) | **Moderate** — §12.1's cloud-provider rung; reversal re-does infrastructure only, touching no stored data, schema, or published contract | ADR-005, ADR-010, ADR-021, ADR-023, ADR-025, ADR-039, ADR-045 |
| **047** | Consolidated gate placement — the CI-level Merge Gate as the pipeline's own gate, diff-based checks run once at Merge, and Production's no-bypass rule made structural by the singular deploy credential | `02-ci-cd-pipeline-design.md` §12.1 | 🔶 **Provisional** (H36) | **Low** — §12.1's cheapest rung; reversal re-authors pipeline configuration only | ADR-005, ADR-010, ADR-014, ADR-021, ADR-022, ADR-023, ADR-025, ADR-032, ADR-035, ADR-045, ADR-046 |
| **048** | The Isolation and Trust module's higher bar (100% branch coverage + a boundary-crossing test class), the tier-applicability enforcement model, and no new evidence type | `03-testing-and-quality-gates-design.md` §12.1 | 🔶 **Provisional** (H37) | **Low** — §12.1's cheapest rung; every decision is a threshold, floor, or enforcement point that reversal retunes | ADR-014, ADR-020, ADR-021, ADR-025, ADR-032, ADR-035, ADR-047 |
| **049** | Tag-based severity attribution fixed at signal-definition time (`severity_source`), tier applicability as deploy-time configuration with no evaluation branch, and the health-signal payload handed to two not-yet-written reaction mechanisms | `04-observability-and-monitoring-design.md` §14.1 | 🔶 **Provisional** (H38) | **Moderate** — telemetry already recorded under one instrumentation shape cannot be re-observed under another after the fact; that asymmetry, not migration cost, is what lifts it above Low | ADR-020, ADR-025, ADR-038, ADR-046, ADR-048 |
| **050** | Region-sequential Production rollout (never atomic all-region cutover), the mandatory rollback path made real by a re-confirmable reversal procedure, and INV-08's release-time reinforcing check placed inside the existing Deploy Gate row | `05-release-and-rollback-design.md` §15.1 | 🔶 **Provisional** (H39) | **Moderate** — pipeline and procedure configuration, not stored schema; above Low because an executed rollout's own sequencing is a historical fact already recorded under that shape | ADR-025, ADR-032, ADR-045, ADR-046, ADR-047, ADR-048, ADR-049 |
| **051** | Incident severity as a *read* of the already-resolved signal severity (never recomputed), containment as confinement to already-structural boundaries, and restore as a mechanism distinct from rollback | `06-incident-response-and-recovery-design.md` §13.1 | 🔶 **Provisional** (H40) | **Moderate** — touches no stored schema, but a past incident's response is already recorded under one severity-inheritance and containment shape and is not re-derivable under a revised one | ADR-025, ADR-045, ADR-046, ADR-047, ADR-048, ADR-049, ADR-050 |
