...
A navigational index of every design-phase ADR: its identifier, status, owning document, cost to reverse, and the upstream decisions it assumes. **Last updated 2026-08-07** (post-H28 — ADR-039 registered at its own ticket's close; ADR-038 was the first under `PROCESS.md` §3's new rule. Prior entry: post-H26c sync, ADR-024 through ADR-037 added. The register had drifted **fourteen** ADRs behind in a single day, having been synced at H15 the day before. See live issue 8.)
...
**Nineteen ADRs now await lead approval — ADR-021 through ADR-039.** The earlier claim here — that every ADR was closed and nothing awaited approval — was true of the Layer 1 set at H6 and has not been true of the phase since H13. The three ADRs that waited on the data model (001, 008, 009) remain discharged by H6 on the evidence of `02-platform-data-model-design.md`.
...
| 🔶 **Provisional — Pending Lead Approval** | **19** | **ADR-021**–**ADR-039** *(H13 through H28)* — every ADR produced since H13, none in `DECISIONS.md` |
...
**Nineteen ADRs now await lead approval — ADR-021 through ADR-039, none recorded in `DECISIONS.md`.** That is every ADR produced since H13, and it is the design phase's largest standing decision backlog. See live issue 8 on why the register keeps falling behind.
