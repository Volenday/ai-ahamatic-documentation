...
| `technology-stack-selection-criteria.md` | Questions-and-criteria set | The reusable question "which technology stack fits this client's circumstances," distilled into a criteria table a future engagement applies directly — not a recommendation for this platform. The most mature candidate: `01-technology-stack-design.md` §18.6–§18.10 already produced a worked default-plus-criteria table for one decision; this document generalizes the criteria themselves, stripped of this platform's own answer. | `docs/design/01-design-foundation/01-technology-stack-design.md` §2.1 (the six original criteria) and §2.6 (criteria 7–13, added post-evaluation), §18.6–§18.10 (the worked instance) | **Done** |
| `workflow-engine-tool-opinion.md` | Third-party tool opinion | A standing position on the class of BPMN-conformant workflow engines: the criteria by which a client's circumstances point to one engine over another, and the constraints any candidate must satisfy (tenant isolation, the portable-subset rule, not becoming a second authoritative data store). `DECISIONS.md` **D-20** committed to adopting an engine rather than building one and explicitly left the engine choice to a separate evaluation — this is that evaluation's home. No engine is named as a decision by this document; a named example (Camunda) was given illustratively in D-20 and is not a conclusion. | `DECISIONS.md` D-20; `docs/design/03-data-construction-contracts/04-workflow-and-process-automation-design.md` (once written, for the constraints an adopted engine must satisfy) | **Done** |
| `ui-component-foundation-tool-opinion.md` | Third-party tool opinion | A standing position on the class of **UI component-and-styling foundations** for a web application surface, scoped as **three separable layers** — component behavior, styling mechanism, and token/theming — each carrying a different reversal cost. Holds the criteria a client applies, four class-level constraints (licence, rendering-approach compatibility, accessibility conformance, and whether visual identity is a parameter or a property), the dimensions candidates legitimately differ on, and a dated survey built to be replaced without touching §§1–6. **Names no product as a decision**, on `workflow-engine-tool-opinion.md`'s precedent. | `docs/design/01-design-foundation/01-technology-stack-design.md` §2.1, §2.6 (criteria vocabulary), §17.6 (ADR-007's Approved surface-shape part); `docs/design/02-identity-access-security/09-licensing-and-dependency-compliance-design.md` §3–§7 | **Done** |
