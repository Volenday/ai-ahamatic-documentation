...
- **Every touched artifact's recorded provenance state is builder-approved.** Per `02-governance-and-security/02-security-policy.md` §5, an artifact the AI-assisted builder tooling (C-19) generates is untrusted until validated, and its platform origin confers it no trust before it crosses any boundary; `03-software-and-architecture/02-domain-glossary.md`'s C-19 entry draws the same distinction — an AI output is a suggestion until the builder confirms it into a builder-approved artifact. That state is a recorded property of each artifact a change touches, independent of whether any review of the change was performed or performed correctly, and a change touching any artifact not recorded as builder-approved does not merge.
