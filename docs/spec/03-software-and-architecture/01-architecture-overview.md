...
This is a Design-phase artifact. It inherits its framing from the Vision and Charter and is subordinate to it; where it appears to conflict with the charter, the charter prevails. It is the structural expression of the primitive taxonomy and the primitive/artifact line defined in `01-business-and-ux/03-platform-capability-model.md`, and it must hold every invariant of `02-governance-and-security/01-system-invariants.md` — chiefly **INV-05 (generality preservation)** and **INV-06 (builder/built separation)**, the two invariants its structure exists to make architecturally true — without redefining either. It cites, rather than re-derives, the canonical capabilities (C-01–C-26) and release gates (G-1–G-6) of `01-business-and-ux/02-prd.md`, and it is consistent with, and cites rather than redefines, every rule of `02-governance-and-security/02-security-policy.md`, `02-governance-and-security/03-access-control-and-tenancy-model.md`, `02-governance-and-security/04-auth-and-identity-spec.md`, and `02-governance-and-security/05-compliance-and-data-residency.md`. Every structure described here is **platform-level and domain-neutral** — valid for any software built on the platform, in any tenant and any region.
...
- **Across both layers.** The model describes the platform's own primitives and the boundary that separates them from every built artifact; the builder/built separation of `01-business-and-ux/03-platform-capability-model.md` §3 is preserved throughout — no component absorbs builder-defined content, and no primitive is made to depend on a builder-defined artifact.
- **Across every lifecycle phase.** The model binds Strategy, Guardrails, Design, Execution, and Evolution alike — a structural boundary drawn here is not a Design-time convenience, it is the same boundary `02-governance-and-security/01-system-invariants.md` §5 holds continuously.
...
A **structural component** is the architectural counterpart of a primitive family (`01-business-and-ux/03-platform-capability-model.md` §4): a single, addressable part of the platform core responsible for the guarantees of one family and no other. A component is defined by three properties:

- **Bound to exactly one primitive family.** A component holds the capabilities of one family (`01-business-and-ux/03-platform-capability-model.md` §4) and does not absorb the responsibility of another.
...
Distinct from a component is the **guardrail layer** — the invariants, security posture, access and tenancy rules, authentication and identity rules, and residency obligations of `02-governance-and-security/01-system-invariants.md`, `02-governance-and-security/02-security-policy.md`, `02-governance-and-security/03-access-control-and-tenancy-model.md`, `02-governance-and-security/04-auth-and-identity-spec.md`, and `02-governance-and-security/05-compliance-and-data-residency.md`. The guardrail layer is not a component: it holds no capability of its own and belongs to no single primitive family. It constrains how every component in §4 discharges the capabilities it holds, and no component may be designed as though the guardrail layer did not apply to it.

---

## 4. Major Components and Their Responsibilities

The platform core comprises seven structural components, one per primitive family (`01-business-and-ux/03-platform-capability-model.md` §4). Each is domain-neutral and holds only the capabilities its family holds; capability IDs are cited from `01-business-and-ux/02-prd.md` §4, not restated.

| Component | Responsibility | Capabilities It Holds | Grounded In |
|---|---|---|---|
| Isolation and Trust | Establish who an actor is, govern what they may do, and keep independent builders strictly separated on shared infrastructure. | C-01, C-02, C-03 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Construction | Let builders construct applications and model their own data, entities, and schemas, and configure structure, behavior, and access — bound to no predetermined domain. | C-04, C-05, C-06 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Operation | Run built software, carry a built application through build, configuration, publishing, and operation as one continuous governed flow, and observe the real-world health of the platform and built software. | C-07, C-08, C-09 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Reach | Carry built software to its intended end users through publishing, and make published software and extensions discoverable and obtainable. | C-10, C-13 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Extension | Extend the platform through modules and a stable programmatic contract, without weakening core guarantees or absorbing domain content into the core. | C-11, C-12 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Distribution | Operate across regions while honoring the obligations that attach to where data and users reside. | C-14 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
| Evolution | Change the platform and built software over time — safely, recoverably, and on a managed path — without breaking prior guarantees. | C-15, C-16, C-17 | `01-business-and-ux/03-platform-capability-model.md` §4, §4.1 |
...
A component may depend only on the components its family depends on. The authoritative relation is the family-level ordering of `01-business-and-ux/03-platform-capability-model.md` §5.2, restated here as the components' allowed dependency direction:
...
| A component instance serving one tenant depending on, calling into, or referencing another tenant's instance or data. | The direct breach of INV-01 (tenant isolation, `02-governance-and-security/03-access-control-and-tenancy-model.md` §3); this holds even between two components whose family-level dependency is otherwise allowed. |
...
|---|---|---|---|
| Platform core | The seven components of §4 and the guardrail layer of §3 — every platform-provided primitive (`01-business-and-ux/03-platform-capability-model.md` §2). | The platform. | A governed platform change; must preserve every prior guarantee (INV-08, INV-09). |
...
|---|---|---|
| The seven-component structure and its dependency ordering (§4, §5.1). | `01-business-and-ux/03-platform-capability-model.md` §4–§5 | The ordering already resequences the PRD's own capability dependencies (`01-business-and-ux/02-prd.md` §4); changing it here would be changing that canonical sequencing, not making a Design-phase choice. |
...
| The trust boundaries of `02-governance-and-security/02-security-policy.md` §3.2 (tenant, authorization, builder/built, extension, region, built-application surface). | `02-governance-and-security/02-security-policy.md` §3.2 | These are the exact points where the security posture applies its controls; relocating or merging one changes what the posture defends without changing the posture's rules. |
...
| Charter precedence and the halt-and-escalate rule for any conflict this model cannot resolve. | `02-governance-and-security/01-system-invariants.md` §3, §6; `01-business-and-ux/01-vision-and-charter.md` | Where a proposed component or dependency cannot be reconciled with §4–§6, the correct action is to halt and escalate, never to resolve the conflict by relaxing a component's boundary. |

How a change to any decision above is proposed, reviewed, and approved is owned by `05-meta-operations/04-human-in-the-loop-protocol.md`, `05-meta-operations/07-self-correction-and-fallback-protocol.md`, and `05-meta-operations/01-agent-operating-charter.md`. This document defines only which decisions require that governed path, not how the path itself operates.

---

## 8. Precedence and Ownership Boundaries

When an element of this model meets any other consideration, it is resolved by the fixed precedence of `02-governance-and-security/01-system-invariants.md` §6.
...
- **Canonical vocabulary and disallowed synonyms** for the concepts named here are owned by `03-software-and-architecture/02-domain-glossary.md`.
- **Data-model and referential rules**, entity/schema definition and validation contracts, and migration-safety constraints are owned by `03-software-and-architecture/03-data-model-and-entity-spec.md`.
- **API versioning, backward-compatibility, and contract-change sign-off** are owned by `03-software-and-architecture/04-api-contract-spec.md`.
- **Extension/module boundary mechanics, SDK compatibility, marketplace submission constraints, and sandboxing** beyond the trust boundary named in §5.2 are owned by `03-software-and-architecture/05-integration-and-extensibility-spec.md`.
- **Numeric thresholds** — performance, scalability, availability, and latency budgets — are owned by `03-software-and-architecture/06-non-functional-requirements.md`.
...
- **Tenant isolation, role and permission, authentication, session, secrets, and residency particulars** already cited are owned by `02-governance-and-security/03-access-control-and-tenancy-model.md`, `02-governance-and-security/04-auth-and-identity-spec.md`, `02-governance-and-security/02-security-policy.md`, and `02-governance-and-security/05-compliance-and-data-residency.md`.
...
- **Enforcement and escalation mechanics** — how a halt is carried out and how a structural change is reviewed and approved — are owned by `05-meta-operations/04-human-in-the-loop-protocol.md`, `05-meta-operations/07-self-correction-and-fallback-protocol.md`, and `05-meta-operations/01-agent-operating-charter.md`.
