...
| Integration and API Management | C-11, C-12, C-24, C-25 | `integration-and-extensibility-spec.md`, `api-contract-spec.md` | Aligned at model level — connector means now provided by C-24/C-25; populated breadth remains a maturity shortfall (`competitive-landscape.md` §4.4) |
...
| Embedded AI/ML and Agentic AI Orchestration | C-19 | The Meta-Operations domain — `agent-operating-charter.md`, `human-in-the-loop-protocol.md`, `agent-loop-constraints.md`, and related | Aligned (build-time) + Differentiation (agent-operated lifecycle); runtime builder-facing orchestration recorded as a future capability **C-26 (not yet authorized)** — a recorded future direction, not delivered coverage (`competitive-landscape.md` §4.2, §5.2) |
...
These benchmarks are the market condition the Enterprise LCAP category exists to address. They anchor the platform's ambition and set the direction of its success metrics; they are reused here for consistency with `value-proposition-and-success-metrics.md` §2.1 and are never converted into internal numeric thresholds. The figures below — the ≈85 million-by-2030 developer shortage, the 70–75% low-code/no-code adoption forecast, and the ≈four-in-five strategic-pillar rate — are **unverified, secondary, and directional**, not licensed or authoritative Gartner findings (`PROCESS.md` §7); they indicate direction, not settled fact.
...
|---|---|---|---|
| Runtime builder-facing agentic orchestration | Critical Capability — Agentic AI Orchestration | Resolved — recorded as future | Recorded as the future capability **C-26 (Runtime AI Automation)** in the formal Future / Not-Yet-Authorized Capabilities category, distinct from C-19 build-time assistance (`competitive-landscape.md` §4.2); a tracked future position, no longer an open gap |
| Cross-system data unification (data fabric) | Critical Capability — Architecture, Integration | Resolved — active | Provided by **C-24 (Cross-System Data Layer, active)** (`competitive-landscape.md` §4.1) |
| Robotic/desktop process automation and BOAT convergence | Market Guide — BOAT (workflow, RPA, iPaaS, low-code convergence) | Resolved — declined | Deliberately **declined** as a web-focused positioning decision (gap review G-3; `competitive-landscape.md` §4.3, §2.3), never a shortfall; workflow is covered by C-18, and the integration/convergence aspects are addressed by C-24 and C-25 |
| Integration and ecosystem breadth | Inclusion threshold — Integration; Critical Capability — Integration and API Management | Standing market-reality (maturity) | The means exist (C-11, C-12, C-13) and C-25 strengthens them; the populated catalog accrues after launch (`competitive-landscape.md` §4.4). Inherent to a platform not yet built, not a pre-launch remediation item |
