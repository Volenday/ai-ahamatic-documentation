...
-  **Across both layers.** The model governs the platform's own primitives and every **built artifact** produced with them; the builder / built separation of `01-business-and-ux/03-platform-capability-model.md` §3 is preserved throughout — no residency rule absorbs a built artifact's obligations into the platform core, and no primitive is made to depend on a builder-defined exemption from them.
-  **Across every lifecycle phase.** The model binds Strategy, Guardrails, Design, Execution, and Evolution alike, exactly as INV-07 does (`02-governance-and-security/01-system-invariants.md` §5) — not only at deployment into a region or at release.
-  **Across every tenant and every region.** Residency and compliance obligations hold identically for each tenant and in each operating region. They may add to the guarantees of `02-governance-and-security/03-access-control-and-tenancy-model.md` and `02-governance-and-security/04-auth-and-identity-spec.md` but never subtract from them, and those documents' guarantees never excuse a residency obligation this document sets.
...
-  **Uncertainty resolves to refusal.** Where it cannot be established that an action honors the residency obligations that attach to it, the action is refused rather than allowed, matching the deny-by-default rule of `02-governance-and-security/01-system-invariants.md` §3.
...
-  **The region boundary is honored as its own trust boundary.** The region boundary of `02-governance-and-security/02-security-policy.md` §3.2 is distinct from, and coexists with, the tenant boundary; honoring one is never a substitute for honoring the other, and a cross-region action must satisfy residency obligations even where the tenant boundary is fully respected.
...
| Any action where the applicable region obligation or compliance regime cannot be established with confidence. | Extends the deny-by-default rule of `02-governance-and-security/01-system-invariants.md` §3 to the approval gate itself: uncertainty is resolved by requiring approval, not by proceeding.                                                |
| A change to how a compliance regime is recognized or honored by the platform.                                 | Bears directly on INV-07, exactly as a change to a trust boundary bears on the mandatory security-review trigger of `02-governance-and-security/02-security-policy.md` §7.                                                                |

This document defines what must be approved before proceeding. How the approval is requested, routed, and recorded is owned by `05-meta-operations/04-human-in-the-loop-protocol.md`.
...
-  **The data-handling matrix built atop these tiers** — retention, deletion, consent, minimization, and redaction — is owned by `02-governance-and-security/06-data-governance-and-privacy.md`.
-  **Tenant isolation particulars** behind INV-01, and client/application scoping, are owned by `02-governance-and-security/03-access-control-and-tenancy-model.md`.
...
-  **Audit, attribution, and tamper-evidence** for residency-consequential actions are owned by `02-governance-and-security/07-audit-and-traceability.md`.
-  **Numeric thresholds** are owned by `03-software-and-architecture/06-non-functional-requirements.md`.
-  **Enforcement mechanics** — how a halt is carried out and how an approval or escalation is requested, routed, and recorded — are owned by the Meta-Operations documents this model feeds, chiefly `05-meta-operations/04-human-in-the-loop-protocol.md`.
