...
- **What the platform steward's internal roles are** — the plane `01-business-and-ux/04-personas-and-roles.md` left unelaborated.
...
- **Across both layers.** The model governs entry to the platform's own primitives and to every **built artifact** produced with them; the builder / built separation of `01-business-and-ux/03-platform-capability-model.md` §3 is preserved throughout — no access rule absorbs a built artifact's access policy into the platform core, and no primitive is made to depend on a builder-defined grant.
- **Across every lifecycle phase.** The model binds Strategy, Guardrails, Design, Execution, and Evolution alike, exactly as INV-01 and INV-02 do (`02-governance-and-security/01-system-invariants.md` §5) — not only at a release or at the moment an actor logs in.
- **Across every tenant and every region.** Isolation and access hold identically for each tenant and in each operating region. Per-region residency obligations (owned by `02-governance-and-security/05-compliance-and-data-residency.md`) may add to this model but never subtract from it.
...
A **tenant** is the account-level boundary within which one builder's members, roles, built applications, configuration, and data exist. Every builder persona and every artifact it builds belongs to exactly one tenant at a time (`01-business-and-ux/04-personas-and-roles.md` §7); every end-user persona belongs to exactly one built application within exactly one tenant.
...
`01-business-and-ux/04-personas-and-roles.md` §2 names the **platform steward** as the apex actor that owns and operates the platform's primitives, and defers the steward's internal role taxonomy to this document. The following roles divide the steward's work; they are facets of the single steward persona already named at the apex of the hierarchy (`01-business-and-ux/04-personas-and-roles.md` §5), not additional personas, and every one of them remains bound by the steward's boundary already set there: governing the tenant boundary without breaching it, and never absorbing builder-defined content into the core.

| Steward Role | What It Governs | Grounded In | Hard Boundary It May Never Cross |
|---|---|---|---|
| Platform governance | The invariants, policies, and release gates that bound every tenant and every built artifact; authorizes what may enter the platform core. | `02-governance-and-security/01-system-invariants.md` §3–§4; `01-business-and-ux/02-prd.md` §5 | Cannot relax an invariant, or admit domain content or a builder-defined dependency into the core (INV-05, INV-06). |
| Tenant admission | Admits and removes tenants at the platform boundary, instantiating each tenant owner's authority. | `01-business-and-ux/04-personas-and-roles.md` §5 | Cannot act as, or in place of, a tenant owner once a tenant is admitted. |
| Platform operations | Owns and operates the platform-provided primitives (C-01–C-27) themselves — distinct from operating any tenant's built software, which is the operator persona's role within a tenant (C-07). | `01-business-and-ux/03-platform-capability-model.md` §2 | Cannot reach into a tenant's built application beyond what operating the shared primitive requires, and cannot bypass isolation to do so. |
| Platform security | Holds the authority to trigger and conduct the mandatory security reviews of `02-governance-and-security/02-security-policy.md` §7, and the halt-and-escalate authority of `02-governance-and-security/01-system-invariants.md` §3 at the platform level. | `02-governance-and-security/02-security-policy.md` §7 | Cannot resolve a review by weakening an invariant, the security posture, or this model. |
...
Tenant admission (row above) is deliberately thin: a steward's admission decision, once made, is the platform-boundary event this model governs, not a richer process this model designs around it. Three further facts state that thinness explicitly, so it is a deliberate boundary rather than an implied one:

- **What precedes the decision to admit is out of scope.** No self-service signup, public registration flow, or business/legal qualification process is modeled in this library (`DECISIONS.md` D-56, D-60). This document states only that a steward's admission decision is the platform-boundary event; it neither designs nor names whatever commercial process leads a steward to make it.
- **Admission requires no criteria beyond the steward's own existing authority.** The Tenant admission role already holds the authority to admit a tenant (row above); exercising it is not gated on any additional qualification this model states. Provisioning is thin and mechanical by design, never a business process wearing a technical mechanism's name.
- **Admission establishes the tenant owner's own first identity and its top authority in one act.** Admitting a tenant creates the tenant boundary of §3 and, within the same act, establishes the tenant owner's own first `platform.platform_users` identity record — the global identity anchor, `docs/design/01-design-foundation/02-platform-data-model-design.md` §3.1 — and grants that owner the top-authority scope §5's matrix fixes for the tenant-owner role, within the newly admitted tenant boundary alone. This is the same event the "Tenant establishment" journey already names — *"Being admitted to the platform, receiving an isolated tenant boundary, and holding top authority within it before anything is built"* (`01-business-and-ux/05-user-journeys.md`) — and the same event the schema-creation mechanics already realize as a data-layer event (`docs/design/01-design-foundation/04-scalability-availability-and-performance-design.md` §5.3). How the tenant owner first authenticates against that identity record — the credential, invite, or sign-in mechanism — is not this document's concern; it belongs to the authentication and identity design (`docs/design/02-identity-access-security/03-authentication-and-identity-design.md`), cited here and not designed.

---

## 5. Role-and-Permission Matrix

This matrix formalizes the permission expectations `01-business-and-ux/04-personas-and-roles.md` §6 set for the access-control model to define, expressed as the scope each permission is bound to. It introduces no persona beyond those already canonical in `01-business-and-ux/04-personas-and-roles.md` §3–§4 and the steward roles of §4 above; capability IDs are cited from `01-business-and-ux/02-prd.md` §4 rather than restated.

| Role | Permission Scope (What It May Do) | Scope Boundary | May Grant Onward To |
|---|---|---|---|
| Platform governance, Tenant admission, Platform operations, Platform security (§4) | Govern the platform's primitives (C-01–C-27), invariants, and boundaries. | Above all tenants. | Tenant owners, by admission. |
...
- **An end-user role is always scoped to exactly one application.** Being an authenticated end user, public consumer, or end-user administrator of one application confers no standing in any other application, even within the same tenant (`01-business-and-ux/04-personas-and-roles.md` §7).
...
| An extension or client carrying a grant from one tenant's context into another's. | The extension boundary (`02-governance-and-security/02-security-policy.md` §3.2) exists precisely to prevent a shared instrument from becoming a cross-tenant path. |
| A platform steward role observing or altering tenant content beyond what governing the boundary requires. | The steward governs the boundary without breaching it (`01-business-and-ux/04-personas-and-roles.md` §6); governance authority is never license to enter a tenant. |
...
Any realized crossing of a tenant boundary is an INV-01 violation, corresponds to the "cross-tenant compromise" threat category of `02-governance-and-security/02-security-policy.md` §3.3, and is a blocking violation that halts and escalates per `02-governance-and-security/01-system-invariants.md` §3 — in whatever lifecycle phase it is found, not only at release.

---

## 8. Least-Privilege Default

Least privilege and deny-by-default are the standing posture of every persona and every steward role in this model, elaborating the defaults `01-business-and-ux/04-personas-and-roles.md` §6 set.
...
- **Grants never exceed the grantor.** Every grant in the matrix of §5 flows downward through the hierarchy of `01-business-and-ux/04-personas-and-roles.md` §5; no role can confer authority it does not itself hold, and no scoping unit of §6 can widen a grant beyond what its containing scope already holds.
...
- **Uncertainty resolves to denial.** Where it cannot be established that an actor holds a permission, the action is refused rather than allowed, matching the deny-by-default rule of `02-governance-and-security/01-system-invariants.md` §3.
...
- **Secrets handling, threat model, and vulnerability-severity thresholds** are owned by `02-governance-and-security/02-security-policy.md`; this document does not redefine severity or the release-blocking threshold.
- **Per-region residency and jurisdictional obligations** are owned by `02-governance-and-security/05-compliance-and-data-residency.md`.
- **Audit, attribution, and tamper-evidence** for access-consequential actions are owned by `02-governance-and-security/07-audit-and-traceability.md`.
- **Enforcement mechanics** — how a halt is carried out and how an escalation is requested, routed, and recorded — are owned by the Meta-Operations documents this model feeds, chiefly `05-meta-operations/04-human-in-the-loop-protocol.md`.
