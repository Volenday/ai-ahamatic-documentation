...
This is a Strategy-phase artifact. It inherits its framing from the Vision and Charter and is subordinate to it; where it appears to conflict with the charter, the charter prevails. It references the canonical capabilities (C-01–C-26) and release gates (G-1–G-6) defined in `01-business-and-ux/02-prd.md`, the primitive families defined in `01-business-and-ux/03-platform-capability-model.md`, and the personas defined in `01-business-and-ux/04-personas-and-roles.md`, rather than re-enumerating them. Every metric here is **platform-level and domain-neutral** — valid regardless of what software is built on the platform, and never satisfied by the success of any single thing built with it.

Concrete numeric thresholds — the exact target values, regression floors, coverage percentages, and alert levels — are owned by `03-software-and-architecture/06-non-functional-requirements.md`, `04-devops-and-cloud-infra/03-testing-and-quality-gates.md`, and `04-devops-and-cloud-infra/04-observability-and-monitoring.md`. This document defines each metric as a measurable quantity with a defined direction and bounded target state; it does not set the numeric thresholds those documents own.

Success here is framed around the **professional builder** — the platform's sole builder audience (charter §2; `01-business-and-ux/04-personas-and-roles.md` §3) — and never around citizen-developer or business-technologist adoption, which the charter excludes by design. The platform's value and the ambition of its metrics are anchored to publicly-available Enterprise LCAP market benchmarks (§2.1); those benchmarks are **external market context**, not the internal numeric thresholds owned by the documents named above. That benchmark material is **publicly-available / secondary data, not a licensed Gartner report**, and is therefore **directional, not authoritative** (`PROCESS.md` §7) — never a licensed or settled Gartner finding. No Magic-Quadrant placement is asserted for AI ahaMatic as a Gartner finding, and none appears here.
...
Success metrics measure how well the platform delivers the means. They never measure, and are never satisfied by, the value of any one artifact built with it. That value is delivered to the **professional builder** — the platform's sole builder audience (charter §2; `01-business-and-ux/04-personas-and-roles.md` §3) — whose productivity in building, operating, publishing, and evolving software the platform's means exist to amplify. Success is framed around that productivity, never around adoption of the platform by non-specialist users.

### 2.1 Market Context

The platform's value answers a market condition the Enterprise LCAP category exists to address. Publicly-available Enterprise LCAP market benchmarks frame that condition and anchor the platform's value. The figures below — the ≈85 million-by-2030 developer shortage, the 70–75% low-code/no-code adoption forecast, and the ≈four-in-five strategic-pillar rate — are **unverified, secondary, and directional**, not licensed or authoritative Gartner findings (`PROCESS.md` §7); they indicate direction, not settled fact:

- **A structural shortage of professional building capacity.** A projected developer shortage reaching roughly 85 million unfilled roles globally by 2030. The platform's value is that it multiplies what the professional builders who remain can produce — letting each build, operate, publish, and evolve far more software than hand-coding allows.
- **A decisive market shift toward low-code building.** A forecast that 70–75% of new enterprise applications will be built through low-code or no-code means within the current market horizon, with the large majority of enterprises already treating low-code as a core strategic pillar rather than an experiment. The platform competes on that trajectory: its worth is the breadth of software it lets professional builders deliver as the shift completes.
...
- **Platform-level only.** A metric is valid only if it holds for arbitrary, builder-defined subject matter. No metric is satisfied by the success of one application, one tenant, or one domain (charter §5; `01-business-and-ux/02-prd.md` §6.9).
...
These are the outcomes the platform exists to maximize or minimize. They are grouped by the primitive family (`01-business-and-ux/03-platform-capability-model.md` §4) whose value they measure. Each is expressed as a measurable quantity with a defined direction and bounded target state; the concrete numeric target is owned by the documents named in the preamble.
...
These metrics measure guarantees that must hold steady. Each corresponds to a release gate (`01-business-and-ux/02-prd.md` §5) or to the builder / built separation the charter is built on. A guardrail metric is a **floor, not currency**: no success metric in §4 may be improved by a move that degrades any guardrail below. A guardrail has no upside to chase — it is either held or violated.
...
| Builder / built separation integrity | Charter §2, §5; `01-business-and-ux/03-platform-capability-model.md` §3 | Instances of the platform absorbing builder-defined content, or of a primitive being made to depend on a builder-defined artifact, remain at zero. |

Guardrail metrics are the properties the critical-path journeys (`01-business-and-ux/05-user-journeys.md` §5) exist to preserve. A change that would degrade any of them halts and escalates rather than proceeding; the enforcement of these blocking conditions is owned by the Guardrails-phase documents.
...
| Platform / artifact coupling                          | Tighter coupling between the platform and any built artifact can appear efficient while eroding the line that keeps the platform generic.                                                                                                                                                                                                                                            | The invariance of the primitive / artifact line (`01-business-and-ux/03-platform-capability-model.md` §3.3).                                                                  |
...
- **Numeric thresholds are owned elsewhere.** This document defines metrics, directions, and target states; the concrete numeric thresholds are owned by `03-software-and-architecture/06-non-functional-requirements.md`, `04-devops-and-cloud-infra/03-testing-and-quality-gates.md`, and `04-devops-and-cloud-infra/04-observability-and-monitoring.md`.
- **Market benchmarks anchor ambition, not thresholds.** Publicly-available Enterprise LCAP market benchmarks — directional, not authoritative, and never licensed or settled Gartner findings (`PROCESS.md` §7) — justify the platform's value and set the direction and ambition of its metrics; they are external context and are never converted into this document's internal numeric thresholds.
