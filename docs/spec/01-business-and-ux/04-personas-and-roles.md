...
A third plane sits above both: the **platform steward** — the actor that owns and operates the primitives themselves (C-01–C-26). The steward governs the boundary within which all builders and end users operate. Its internal role taxonomy is governed by `02-governance-and-security/03-access-control-and-tenancy-model.md` and the Meta-Operations documents; this document acknowledges it only as the apex of the hierarchy and does not elaborate its roles.
