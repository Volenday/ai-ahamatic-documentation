...
-  **LCAP identity.** AI ahaMatic is formally an **Enterprise Low-Code Application Platform (LCAP)**. Its product is the capability to build and operate applications through high-productivity, model-driven means — not a hand-built application dedicated to a single purpose. Being an LCAP is the generic-builder mandate stated in market terms: an LCAP's worth is the breadth of applications it enables, so the categorization pulls toward domain-neutrality rather than toward any single vertical. Every downstream decision must remain consistent with AI ahaMatic being an application platform, never an application, and the LCAP framing must never be read as license to narrow the platform toward one domain.
-  **LCAP identity versus tier commitment — these are two different things and must not be reconciled by changing either one.** LCAP is the market *category* the platform is classified in, fixed independently of how the platform assembles software; the tier below is the platform's own *assembly* commitment within that category. A platform classified as an Enterprise Low-Code Application Platform that commits to the no-code tier is not a contradiction — the category name is inherited from the Gartner market definition and is not itself a tier claim. No future session may "correct" the LCAP name to match the tier, or the tier to match the LCAP name.
-  **LCNC market positioning.** AI ahaMatic is positioned within the **Gartner-defined Low-Code/No-Code (LCNC) market** and commits explicitly to its **no-code** tier. At charter level the market spans two tiers, distinguished by assembly method:
    -  **Low-code** — applications assembled through high-productivity, model-driven means while retaining the ability to extend with code.
    -  **No-code** — applications assembled entirely through configuration, without code.
-  **Scope of positioning at charter level.** This categorization is the reference point that fixes which market expectations, capability baselines, and standards apply to the platform. Detailed positioning against the market — vendor comparison and standards mapping — is not a charter concern; it is carried by `01-business-and-ux/07-competitive-landscape.md` and `01-business-and-ux/08-industry-standards-and-benchmarks.md`.
-  **Professional-builder audience.** AI ahaMatic is built for **professional builders** — those who build software as a professional discipline. The platform's identity is framed around that audience: its primitives, capabilities, and productivity are designed to serve the professional builder's work of constructing, publishing, and operating software. Audience is fixed here, exclusively; the tiers above are defined by assembly method alone and carry no audience claim of their own.
