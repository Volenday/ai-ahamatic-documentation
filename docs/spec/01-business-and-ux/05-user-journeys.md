...
Journeys divide along the same line that separates the two persona layers in `01-business-and-ux/04-personas-and-roles.md` and the primitive / artifact line in `01-business-and-ux/03-platform-capability-model.md`. A journey is classified by **what the actor acts on**:
...
Build-time journeys are the experiences of the builder personas. They follow the family-level ordering of `01-business-and-ux/03-platform-capability-model.md` §5.2: each journey assumes the capabilities the journeys before it establish. The journeys below are the canonical build-time experiences; a single builder may travel several, and not every tenant travels every one.
...
| Data administration | Application builder | Administering the records held under its own entities — creating, reading, updating, and deleting them through a generic administrative interface derived automatically from the builder's entity definitions, before any application has been built. | C-27 |
...
A critical-path journey is one whose failure would break a guarantee the platform's release depends on. These journeys map directly to the release gates in `01-business-and-ux/02-prd.md` §5. A change that causes any of them to regress may not proceed — the gate is a hard constraint, not a preference. Each critical path applies across **every** build-time and run-time journey above; it is a property the whole experience must preserve, not a separate journey traveled once.
...
- **A data model with entities defined but no application yet built** exists once a builder has modeled entities (C-05) but before any application has been constructed; C-27's generic administrative interface, derived automatically from those entity definitions, is usable and presents no records to create, read, update, or delete. This is a valid empty state, not a failure, and it is structurally distinct from the built-application empty state below: C-27 operates before an application exists at all, which is exactly its boundary against C-06 and C-07.
