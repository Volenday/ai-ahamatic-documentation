...
| C-05 | Data and entity modeling | The means for builders to define their own data, entities, and schemas, with rules that keep those definitions valid, consistent, and related correctly. It generically supports effective dating, append-only records, and full history for every entity a builder defines, with every record's history auditable by default — never as an opt-in for particular domains. | C-04 |
...
| C-24 | Cross-system data layer | The means for built software to access, integrate, and operate over data residing across multiple or external systems through a unified, domain-neutral data-access layer, bound to no predetermined domain — defining the unified access surface that presents disparate external and platform-resident data as one coherent layer; the guarantee that data drawn across systems remains strictly tenant-scoped (INV-01); the rule that credentials used to reach external systems are never exposed in code, configuration, logs, or stored state (INV-03; detailed handling under `02-governance-and-security/02-security-policy.md` §4); the treatment of all data drawn from external systems as untrusted input validated at the boundary (`02-governance-and-security/02-security-policy.md` §5); the obligation that cross-system data movement honors the residency and jurisdictional obligations of where data resides (C-14; `02-governance-and-security/05-compliance-and-data-residency.md`); and the explicit boundary that separates this cross-system data-access capability from both a builder's modeling of their own entities and schemas (C-05) and the platform's module and programmatic-contract extension mechanisms (C-11, C-12), neither of which it absorbs or replaces. | C-04, C-05, C-06 |
| C-25 | Connector marketplace | The means to offer, discover, and obtain reusable, pre-built connectors — integrations to external systems — with the platform's guarantees preserved across what is offered, bound to no predetermined domain — defining the connector as a generic integration that admits no domain content into the platform core (INV-05, INV-06); the requirement that a connector, which crosses the extension boundary, triggers the mandatory security review before it is offered (`02-governance-and-security/02-security-policy.md` §3.2, §7; `03-software-and-architecture/05-integration-and-extensibility-spec.md` §6–§7); the guarantee that obtaining a connector never grants cross-tenant reach (INV-01); the rule that credentials a connector uses to reach an external system are never exposed in code, configuration, logs, or stored state (INV-03; `02-governance-and-security/02-security-policy.md` §4); the obligation that every offered connector satisfies the license-category and cumulative-attribution constraints for marketplace submissions (`02-governance-and-security/08-legal-and-licensing-constraints.md`, as applied in `03-software-and-architecture/05-integration-and-extensibility-spec.md` §6); the explicit boundary that separates this connector marketplace from the general marketplace (C-13) — which offers published software and extensions at large, of which C-25 is neither a subset nor a rename, neither absorbing the other; and the explicit boundary that separates it from the cross-system data layer (C-24) — connectors obtained through C-25 supply the external-system connections that C-24 accesses and unifies, C-25 being the discovery and distribution of connectors and C-24 the unified data-access layer over connected systems, neither absorbing the other. | C-13, C-24 |
