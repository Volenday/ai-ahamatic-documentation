...
This model inherits its framing from the Vision and Charter and is subordinate to it; where this document appears to conflict with the charter, the charter prevails. The capabilities named here are the canonical set defined in the PRD (C-01–C-27); this document organizes and relates them, and cites those identifiers rather than re-enumerating them.
...
| Data administration | The means to create, read, update, and delete records over whatever entities a builder has defined, derived from those definitions and presuming no application. | The specific records a builder maintains under those entities. |
...
| Construction | Create software, model its data and entities, administer the records held under those entities, configure its structure, behavior, and access, model the processes that run across it, and assist professional builders with AI-native tooling across construction — bound to no predetermined domain. | C-04, C-05, C-06, C-18, C-19, C-22, C-26, C-27 |
...
The **authoritative, per-capability dependencies** are the "Depends On" relations defined in `01-business-and-ux/02-prd.md` §4 and are not restated here. This section states the dependency **rules** the agent must respect and the family-level ordering those relations imply.

### 5.1 Sequencing Rules

- **Prerequisite-first.** A capability may not be built, assumed, or relied upon before every capability it depends on holds. The dependency graph in `01-business-and-ux/02-prd.md` §4 is the reference for what depends on what.
...
- **A dependency never points to a higher tier.** Because tier order is build order, every capability's prerequisites lie in its own tier or below it. A dependency on a capability in a higher tier would require the higher tier to hold before the lower one completes, inverting the tier ordering; a proposed dependency that crosses upward is therefore a definition error, corrected in `01-business-and-ux/02-prd.md` §4 rather than accommodated in sequencing.
- **Gates are hard constraints.** The release-gating capabilities (`01-business-and-ux/02-prd.md` §5, G-1–G-6) must hold for a release to proceed. A gate is a non-negotiable sequencing constraint, not a preference; its absence blocks the release regardless of how complete the rest is.
...
The per-capability dependencies imply the following ordering among families. This is a coarse view for sequencing at the level of primitive kinds; the fine-grained order remains the per-capability graph in `01-business-and-ux/02-prd.md` §4.
