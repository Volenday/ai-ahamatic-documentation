...
- **Builder confirmation.** The mandatory human-builder confirmation `03-data-construction-contracts/09-ai-assisted-builder-tooling-design.md` designs has occurred. This document does not design that workflow; it requires only that the workflow's own confirmation is a distinct, necessary condition and is never treated as sufficient on its own.
...
- **The suggestion model, confirmation workflow, and programmatic interface of the tooling itself** — `03-data-construction-contracts/09-ai-assisted-builder-tooling-design.md`'s.
- **The context-assembly technique that packages untrusted content for a model-facing request** — `03-data-construction-contracts/04-prompt-and-context-assembly-design.md`'s.
