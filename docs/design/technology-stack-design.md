...
**Amendment at approval (2026-07-30) — the partition unit is per-application, not per-customer (`DECISIONS.md` D-10).** Approving ADR-004, the lead separated data at **three levels** rather than two: platform-global configuration → per-customer → **per-application**. The hierarchy is *the platform has customers, customers have applications*, and it is the **application**, not the customer, that receives its own set of tables. This is **finer-grained than "schema-per-tenant" as recorded above, and it is not a rename of that same shape** — restated precisely: the schema unit this document recommends is **per-application**, nested under the customer, not per-customer directly.

This carries **two isolation strengths that must never be collapsed into one another**:
- **Customer ↔ customer: absolute.** No customer may observe, affect, or detect the existence of another. This is INV-01, and it does not soften at any scale or release.
- **Application ↔ application within one customer: structural by default, but deliberately bridgeable** via APIs, at the customer's own choice — so a customer may keep their applications siloed for safety or connect them for an internally-integrated suite.
...
**This also sharpens, without resolving, the two risks already flagged above.** Migration fan-out and connection-pool pressure now scale with **application count**, not customer count — a customer with many internally-siloed applications multiplies both risks the same way many customers would. `scalability-availability-and-performance-design.md` and `tenant-isolation-and-access-control-design.md` still own resolving them; this amendment only restates, at the correct granularity, the exposure they must resolve.
...
- **Status:** Approved, with amendments (`DECISIONS.md` D-08, D-10).
...
- **Multi-tenancy separates at three levels, and its two isolation strengths are never collapsed.** ADR-004 (§14.7, amended per §14.5) partitions platform-global configuration → per-customer → per-application, with the application as the schema unit. Customer↔customer isolation is **absolute** (INV-01, non-negotiable at any scale); application↔application isolation within one customer is **structural by default but customer-bridgeable via APIs**. Building either boundary at the other's strength is a defect: it either makes the cross-application access the lead wants impossible, or breaches INV-01.
