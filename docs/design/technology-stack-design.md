...
| Scalability | Fit against the concurrency, throughput, and growth targets of `non-functional-requirements.md` §5. |
...
**Traceability to the fixed architecture.** The recommendation realizes, without altering, the structural decisions `architecture-overview.md` fixes as non-overridable (§7):

- **The seven-component structure and its dependency ordering (§4, §5.1).** None of the three recommended technologies presumes or requires any dependency direction other than what `architecture-overview.md` §5.1 already permits; the stack is a realization layer beneath the component model, not a change to it.
...
- **The trust boundaries of `security-policy.md` §3.2.** Nothing in this recommendation relocates or merges a trust boundary; each of the three technologies is a fully containerizable, general-purpose runtime capable of enforcing the guardrail layer (`architecture-overview.md` §3) through whatever mechanism `security-controls-design.md` and `tenant-isolation-and-access-control-design.md` define.
...
- **Containerization is the mandatory deployment mechanism for every layer of the recommended stack.** The server, the web runtime, and any platform-side build/release tooling for the mobile-delivery runtime are packaged as OCI-compliant container images. This is the mechanism that keeps the architecture portable across cloud providers, per `architecture-overview.md` §7 and the project lead's stated deployment scope for this ticket.
...
- **The specification prevails.** Nothing in this document narrows, expands, or alters `architecture-overview.md`, `non-functional-requirements.md`, `coding-standards-and-patterns.md`, or `legal-and-licensing-constraints.md`; where a recommendation here appears to conflict with any of them, that specification governs and the recommendation is corrected, not the specification.
