...
- **Upstream decisions assumed:** `platform-data-model-design.md` (Element A, §18.6) — the evidentiary input this ADR's re-evaluation was sequenced to wait for (`DECISIONS.md` D-08), now supplied. `DECISIONS.md` D-15 — the reframing that changes what this ADR's content *is*: a default plus a reusable, per-client criteria set, not a platform-wide binding choice.
