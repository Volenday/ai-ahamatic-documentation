...
**Status: Mixed — see the per-ADR status in each Design Decision Record and the summary in §22–§23.** This document originally presented every recommendation as provisional pending lead approval. As of the 2026-07-30 standup (`DECISIONS.md` D-08), six of the eleven decision records here are **Approved** (ADR-004, ADR-005, ADR-006 in part, ADR-007 in part, ADR-010, ADR-011), one sub-decision is **Parked** (GraphQL, inside ADR-006), and three remain **Deferred pending the data model** (ADR-001, ADR-008, ADR-009) — the language-and-framework recommendation of §7 among them. No ADR still marked Provisional, Deferred, or Parked takes effect, and no downstream design document proceeds against it, until the project lead confirms it and it is recorded in `DECISIONS.md`. This document has no authority to finalize a still-open decision on its own.
...
- **Status:** Deferred — pending the data model (`DECISIONS.md` D-08). **Its mobile-delivery component is restored, not superseded**: ADR-007 (§17) had superseded it with Flutter, and ADR-009 (§19) reversed that back to React Native with Expo on verified evidence — ADR-007's replacement of this component has lapsed. Its server and web components were reaffirmed by ADR-008 (§18), whose own closure claim is void and whose re-evaluation is live again (§18.6). Read this ADR together with ADR-003 (§13) and ADR-008 (§18).
- **Cost to reverse:** Low — the cheapest decision in the queue (`PROCESS.md` §12.1; `ADR-REGISTER.md`).
- **Upstream decisions assumed:** None structurally; sequenced after the data model by the lead's own cost-to-reverse ordering (`DECISIONS.md` D-08), so the data model is the evidentiary input this ADR's pending re-evaluation waits on, not a dependency this ADR's content presumes.
- **Verified vs. reasoned:** The server- and web-layer findings (§3–§4, §18) are reasoned from structural properties (language, type system, ecosystem shape) and were not independently source-checked at time of writing. The mobile-layer finding is verified — ADR-009 (§19) checked its ecosystem-maintenance claims against current sources after ADR-007's unverified claims were shown wrong.
...
- **Decision:** Recommend Node.js/TypeScript (server), Next.js/React/TypeScript (web — builder UI and built-application runtime), and React Native/TypeScript with Expo (mobile-delivery runtime, C-20), deployed in OCI containers with Google Cloud Platform as the reference target only, per §7–§8 above.
...
- **Status:** Approved. The criteria adopted here originate from the project lead's own direction — criterion 7 from the 2026-07-28 standup, criteria 8–10 from the lead's decision brief, and criteria 11–13 from the 2026-07-29 standup (§2.6) — so the method change itself requires no separate approval. What remains open is the *re-evaluation it mandates*, not the criteria. **This mandate is live again**: it was believed discharged by ADR-008 (§18.6), but ADR-008's closure claim is void — three further criteria (11–13) arrived on 2026-07-29, after ADR-008 was written, and ADR-008's approval (ADR-003 is itself Approved) still binds a re-evaluation against them.
...
- **Status:** Deferred — pending the data model (`DECISIONS.md` D-08). **Its closure claim is void**: this record states below that "ADR-001 needs no further re-evaluation before approval on the criteria grounds ADR-003 raised." Three further criteria (11–13) arrived on 2026-07-29, after this record was written, and ADR-003 — which is Approved — still mandates a re-evaluation against them (§2.6, §13). The mandate this ADR believed discharged is live again.
- **Cost to reverse:** Low (`PROCESS.md` §12.1; `ADR-REGISTER.md`).
- **Upstream decisions assumed:** ADR-003 (§13) and ADR-007 (§17.6). ADR-007's mobile-runtime component, assumed below, has since been superseded by ADR-009 (§19) — see the Consequences correction below.
- **Verified vs. reasoned:** Reasoned — the `.NET`/Blazor comparison (§18.2–§18.4) argues from structural properties (type system, rendering model, off-the-shelf capability) rather than from time-sensitive ecosystem-maintenance claims; no verification pass was run against current sources for this ADR specifically.
- **Context:** ADR-003 found ADR-001 formed against an incomplete criteria set; the lead's brief ranks `.NET` + Blazor + Flutter first and TypeScript end-to-end with React Native eighth of nine. ADR-007 had already settled the mobile layer **at the time this record was written**, with Flutter as its runtime choice.
- **Decision:** **Reaffirm** ADR-001's server (Node.js/TypeScript) and web (Next.js/React/TypeScript) components under all ten criteria, on the revised rationale of §18.5 — expressly replacing §3's cloud-ecosystem-gravity rationale for excluding `.NET`, which this ADR records as insufficient. ADR-001's mobile component was recorded here as remaining superseded by ADR-007 — **this is now corrected below.**
- **Alternatives considered:** `.NET` (C#) on the server — stronger on criteria 7, 8, 9 and modestly on 10 (§18.2), rejected because Blazor cannot serve arbitrary published web output (§18.3) so the unified-stack benefit is unavailable and the platform's own language count would rise by one, and because its verification advantage is partly neutralised on the runtime-defined data path (§18.4). Blazor Server and Blazor WebAssembly for the web layer — rejected on stateful-circuit and initial-load/indexability grounds respectively, the same reasoning §4 applied to Flutter web. Go — unchanged from §3 and §7; remains a fallback, de-named rather than pre-committed, for a narrow, profiled, CPU-bound component if one is later shown by profiling to need it (`TICKET.md` Q13).
- **Consequences:** **Corrected (2026-07-30, amendment ticket).** This field originally recorded, "With ADR-007, the approved stack becomes Node.js/TypeScript (server), Next.js/React/TypeScript (web), **Flutter/Dart (mobile-delivery runtime)**..." — Flutter is **not** part of the approved stack. ADR-009 (§19) reversed ADR-007's mobile-runtime choice back to **React Native with Expo**, which is also ADR-001's original, restored mobile component; the platform's own client surfaces are single-language TypeScript, not TypeScript-plus-Dart. This field also originally argued adopting `.NET` would take the platform "from two languages to three" — with Flutter no longer part of the stack, **the true comparison is two languages (C#, TypeScript) versus one (TypeScript)**; the direction of the argument survives (the language count still doubles), the stated magnitude does not (§18.3's note). The corrected consequence: PostgreSQL with a typed query builder (ADR-004), one deployable with enforced module boundaries (ADR-005), OpenAPI contracts at both tiers with the GraphQL sub-decision parked (ADR-006), React Native with Expo for mobile (ADR-009), and GCP as the default cloud target (ADR-010). The criterion-7/9 deficit identified in §18.5 imposes a standing obligation on dependency selection, binding `coding-standards-and-patterns-design.md` and `licensing-and-dependency-compliance-design.md`. **This ADR's claim to close the re-evaluation ADR-003 required is void**: criteria 11–13 (§2.6) arrived after this record was written, and ADR-001/008/009 are re-scored against them once the data model exists (`ADR-REGISTER.md`), not reaffirmed on the criteria-7–10 grounds alone.
...
- **Status:** Deferred — pending the data model (`DECISIONS.md` D-08). Supersedes ADR-007's runtime choice; ADR-007's surface-shape decision is unaffected. **Substantively settled but formally deferred**: the lead accepted this recommendation at the 2026-07-30 standup and folded its formal confirmation into the data-model deferral alongside ADR-001 and ADR-008, without re-raising Flutter (`ADR-REGISTER.md`).
...
## 22. Precedence and Ownership Boundaries
...
- **Six ADRs are approved; three remain deferred; one sub-decision is parked (`DECISIONS.md` D-08).** ADR-004, ADR-005, ADR-006 (in part), ADR-007 (surface-shape part), ADR-010, and ADR-011 are **Approved** and downstream documents may cite them as such. ADR-001, ADR-008, and ADR-009 remain **Deferred pending the data model** — a downstream document may cite ADR-001's or ADR-009's content as the recommendation in force (ADR-009's mobile-runtime choice is substantively settled, per its own status note), but may not treat either as formally approved until the data model exists and the lead confirms them. The GraphQL sub-decision inside ADR-006 is **Parked**, pending a pros-and-cons study — no downstream document may treat GraphQL as finally rejected or finally adopted.
...
This document owns the evaluated candidate set, the tradeoff analysis, the recommendation and its approval status, the deployment-substrate rule, and the ADR convention for the design phase. It does not own the specifications it realizes, the isolation and sync-integration mechanics it defers, or the enforcement mechanics of the documents it gates — each remains owned where `implementation-document-map.md` and the cited specifications already place it.

---

## 23. Binding Rules

- **Approval status is per-ADR, not uniform across this document.** ADR-004, ADR-005, ADR-006 (in part), ADR-007 (surface-shape part), ADR-010, and ADR-011 are Approved (`DECISIONS.md` D-08, D-11); ADR-001, ADR-008, and ADR-009 remain Deferred pending the data model; the GraphQL sub-decision inside ADR-006 is Parked. §9's status vocabulary was extended to name all of these (`Approved in part`, `Deferred pending <upstream deliverable>`, `Parked`, `Superseded ... as to <part>`) once approval produced statuses the original three-value set did not cover.
...
- **Five named documents remain gated** on ADR-001 specifically (§11), which remains Deferred; the datastore, architecture, contract-shape, surface-shape, cloud, and sync-posture decisions (ADR-004 through ADR-007, ADR-010, ADR-011) are separately Approved and do not wait on ADR-001's formal confirmation.
...
- **ADR-001's re-evaluation is not complete, and ADR-008's claim that it was is void.** ADR-003 (§13) required a re-evaluation under criteria 7–10; ADR-008 (§18) reaffirmed server and web on that basis and recorded the re-evaluation as closed — but criteria 11–13 arrived on 2026-07-29, after ADR-008 was written, and ADR-003 (Approved) still binds a re-evaluation against them. The mobile layer was reversed to Flutter (ADR-007) and reversed back to React Native with Expo on verified evidence (ADR-009, §19); that reversal stands, but it does not discharge ADR-003's now-live mandate for criteria 11–13. ADR-001, ADR-008, and ADR-009 are re-scored together once the data model exists (`ADR-REGISTER.md`).
...
- **An AI-to-AI interaction protocol is a requirement, with its identity still open.** ADR-006 (§16.5, amended) requires the platform to publish a protocol so AI agents can interact with it; MCP and A2A are the candidates, and this document decides neither. No downstream document may treat either as chosen.
...
- **Every future comparison applies all thirteen criteria.** No stack or architecture comparison on this project is run against §2.1's six alone, nor against the ten of §2.6's first extension; criteria 11–13 (§2.6) — human-verifiability, commercial acceptability, and corpus quality — extend the set again. §3–§5 are not retroactively rescored under any of these extensions, and that non-retroactivity is recorded, not silently assumed.
