...
- **Status:** Approved in part (`DECISIONS.md` D-08). The contract-shape decision (OpenAPI/tRPC/gRPC) is Approved. **The GraphQL rejection is Parked** — reopened pending a pros-and-cons study the lead asked for; it is neither approved nor still rejected until that study reports (§16.3).
- **Cost to reverse:** High (`PROCESS.md` §12.1; `ADR-REGISTER.md`).
...
### 22.2 Two Constraints Binding Whenever a Cache Does Arrive

Deferring the decision does not defer these — they bind whatever caching decision is eventually made, and are recorded now so a future addition does not silently violate them:
...
### 22.3 What This Section Does Not Decide
...
### 22.4 ADR-012 — Server-Side Caching
...
- **Decision:** Defer server-side caching. No caching layer is adopted for V1.0. Whenever one is later introduced, it must (a) never become correctness-critical — the server remains authoritative — and (b) remain within ADR-010's portable subset, never depending on a provider-unique managed cache for correctness.
...
- **Ten ADRs are approved or resolved; one sub-decision is parked (`DECISIONS.md` D-08, D-15, D-16).** ADR-004, ADR-005, ADR-006 (in part), ADR-007 (surface-shape part), ADR-009, ADR-010, ADR-011, ADR-012, and ADR-013 are **Approved/Resolved**, and downstream documents may cite them as such. **ADR-001 and ADR-008 are Resolved as a V1.0 default plus a reusable, per-client criteria set** (§18.6–§18.10), discharging the data-model deferral — a downstream document may build the platform's own V1.0 against the Node.js/TypeScript/Next.js/React-Native default, and a future client engagement may instead apply §18.7's criteria table and §18.8's condition list to choose `.NET` for that client, without either being read as reopening this ADR. The GraphQL sub-decision inside ADR-006 remains **Parked**, pending a pros-and-cons study — no downstream document may treat GraphQL as finally rejected or finally adopted.
