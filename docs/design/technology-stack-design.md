...
- **Upstream decisions assumed:** `platform-data-model-design.md` §3 (the fixed platform schema) — the evidentiary input this ADR's re-evaluation was sequenced to wait for (`DECISIONS.md` D-08), now supplied; discussed here in §18.6. `DECISIONS.md` D-15 — the reframing that changes what this ADR's content *is*: a default plus a reusable, per-client criteria set, not a platform-wide binding choice.
- **Verified vs. reasoned:** The server- and web-layer findings (§3–§4, §18) are reasoned from structural properties (language, type system, ecosystem shape), including the data-model finding of §18.6, which is reasoned from the schema `platform-data-model-design.md` already defines, not independently source-checked. The mobile-layer finding is verified — ADR-009 (§19) checked its ecosystem-maintenance claims against current sources after ADR-007's unverified claims were shown wrong.
...
- **Consequences:** `architecture-realization-design.md`, `scalability-availability-and-performance-design.md`, `licensing-and-dependency-compliance-design.md`, `coding-standards-and-patterns-design.md`, and `environment-and-configuration-design.md` (§11) each realize this default for the platform's own V1.0 build. A downstream document builds the platform against this default; it does not thereby forbid a future client engagement from applying §18.7's criteria table and choosing differently for that client's own build — that choice is exactly what D-15 makes a supported outcome, not an exception requiring this ADR to be superseded.

---

## 11. Documents Gated on This Decision

Per `implementation-document-map.md`, the following design documents depend on `technology-stack-design.md`. **This gate is cleared**: ADR-001 is Resolved (§18.10) and records the V1.0 default these documents build against (§18.6–§18.9; `DECISIONS.md` D-15).
...
- **Isolation mechanics are not owned here.** §14.5 selects a partitioning *shape* only. How tenant isolation is enforced, verified, and proven at the boundary — including the two isolation strengths §14.5's amendment states must never be collapsed — is owned by `tenant-isolation-and-access-control-design.md`, which may overturn the shape on a scalability or enforcement finding; this document does not pre-empt that.
- **Sync posture is decided and does not pre-empt client-side design.** ADR-011 (§21) fixes that the server is authoritative and that one standardised client library pattern is used; the concrete integration of that pattern into any given screen or module is `architecture-realization-design.md`'s and downstream client-facing designs' to realize.
