...
### 22.2 Three Constraints Binding Whenever a Cache Does Arrive

Deferring the decision does not defer these — all three bind whatever caching decision is eventually made, and are recorded now so a future addition does not silently violate them:
...
- **Verified vs. reasoned:** Reasoned — this decision follows from the absence of a demonstrated performance driver and from principles already fixed elsewhere in this document; it makes no time-sensitive claim about any caching technology, because none is selected.
...
- **Decision:** Defer server-side caching. No caching layer is adopted for V1.0. Whenever one is later introduced, it must (a) never become correctness-critical — the server remains authoritative; (b) remain within ADR-010's portable subset, never depending on a provider-unique managed cache for correctness; and **(c) have its licence category checked at the moment of adoption against `02-governance-and-security/08-legal-and-licensing-constraints.md` §4, on then-current evidence (§22.2, added 2026-08-03).** **Valkey is pre-recorded as the presumptive candidate (§22.3) — a pre-recording, not a selection, conferring no authority to introduce a cache.**
...
- **Consequences:** No downstream design document may introduce a caching layer for V1.0. Any future addition must cite this ADR, satisfy both constraints of §22.2, and record its own criteria per `DECISIONS.md` D-16 rather than treating this deferral as blanket permission.
