...
- 🔶 **Tenant provisioning** — needs a lead decision on **richness** (thin: a steward admits, extends the existing tenant-isolation document family; or rich: self-service, billing, KYC, needs a standalone document). The mechanism side (schema, isolation, audit) needs no rework either way. Sequencing note: pairs naturally with builder onboarding's narrow half (already closed by T79) in the same document family.
...
- 🔶 **Application lifecycle** (end-to-end lifecycle of a built application) — needs a lead decision **before anything else can move**, and it is not the library-scope question: `05-user-journeys.md`'s "Safe evolution" journey implies C-15/C-17 cover a built application's own retirement, while the PRD's own C-15/C-17 definitions scope both to the *platform's* evolution, not a builder's application — an internal inconsistency the design work itself cannot resolve. If C-15/C-17 are meant to reach it: amend the PRD's capability definitions, then a new Layer 4 design document. If not: a new, currently-unnamed capability, PRD row first. Sequencing note: `platform.applications` and `platform.tenants` share the identical `active`/`suspended`/`offboarding` enum with an equally undesigned transition mechanism in both cases (per the audit's overlap analysis) — one design pattern may close both once scoped.
...
- 🔶 **AI model governance** — needs a lead decision, before sizing. The library's repeated, deliberate refusal to name a model or vendor is defensible on domain-neutrality grounds, but is in tension with what model-selection criteria, vendor-lock-in mitigation, or deprecation-handling guidance would require discussing, even abstractly. Natural anchor if pursued: alongside `09-ai-assisted-builder-tooling-design.md` and `05-ai-tooling-security-design.md`. Pairs with AI output-quality standards (below) as one scoping conversation, per the audit's overlap analysis — not the same question, but two ends of one currently-undesigned pipe.
- 🔶 **AI output-quality standards** — needs a lead decision, leaning toward a new document or an extension anchored in `09-ai-assisted-builder-tooling-design.md`. The library states its own gap in its own words three separate times (provenance gating is binary confirm/reject, never a correctness judgment) — narrower than most "should this exist" questions because the library already concedes the absence, but a quality *standard* is a "what," so no design-only fix is available until a spec-level statement exists.
- 🔶 **Bias / fairness** — needs a lead decision, and per the audit this is **the one area where the evidence doesn't lean toward an answer at all** — no adjacent mechanism to extend, no in-library statement of why it's absent (unlike AI output-quality's self-disclosed gap). A genuinely open legitimacy question: does AI ahaMatic's build-time tooling (generating code/logic/layout, not decisions about people) carry the bias/fairness risk profile most governance frameworks target, or is this properly left to each client engagement. Domain neutrality and the project's stated avoidance of named regulatory regimes both argue for asking rather than assuming. Treat as a separate, prior question from AI governance/output-quality — no shared citation or mechanism links them.
...
- 🔶 **Accessibility / internationalization** — UI localization is **settled, out of scope** (`PROCESS.md` §5: *"not planned work... must not be resurfaced"*). Accessibility conformance remains genuinely open — a lead decision on whether the platform carries a conformance obligation at all (ADR-060 itself declines to decide this on the spec's behalf, per `01-technology-stack-design.md` §24.2). If yes: a new NFR row or a short new `01-business-and-ux/` document. Pairs with UI/UX design-system coverage (below) only in that one criteria document (CR04) touches both — they are two separate decisions.
- 🔶 **Data migration / import** — needs a lead decision on **capability scope**, then a new document if pursued. Cannot be discharged by extension: the data-model spec's "migration" is definitionally schema evolution of data already held, and both C-24 and C-27 are structurally single-record/per-call by already-settled design decisions. A bulk-import capability, if wanted, needs its own PRD row before any spec or design document can be written against it.
- 🔶 **Offline resilience** — mobile is fully covered (ADR-017). The web/console surface is the open half: ADR-011 itself names the gap and explicitly declines to close it (*"a genuinely offline-first application... may need an escape hatch this decision does not itself design"*). Needs a lead decision on whether web-surface offline resilience is wanted at all before it's ticketed as an extension of ADR-011 or a sibling to the mobile document.
- 🔶 **UI / UX and design-system coverage** — needs a lead decision, then a new `01-business-and-ux/` document if pursued. **Prominence downgraded 2026-08-14 by the audit**: ADR-060 (`01-technology-stack-design.md` §24, landed one day after this note was originally written) selected a component/styling/token foundation for the platform console specifically — relieving the acute D-31 daily-prototyping pressure this entry originally flagged as most blocking. What remains open: whether the platform commits to its own visual identity/brand/typography (as opposed to treating it as a permanently deferred parameter, which is what ADR-060 currently does deliberately), plus the mobile client's and the future UI generator's own, separately-undecided foundations.
- 🔶 **Load testing** — needs a lead decision on **strategy**, then an extension either way (never a new document). The honest boundary, per the audit: is "detect scale pressure in production, not simulate it pre-release" the platform's deliberate, already-implicit strategy (the one concrete scale threshold the design library derives is explicitly assigned to production monitoring, not pre-release testing) — in which case the fix is to *state* that trade-off explicitly — or is a genuine pre-release load-testing gate wanted, anchored at `04-devops-and-cloud-infra/03-testing-and-quality-gates.md` §4 and its design counterpart.
