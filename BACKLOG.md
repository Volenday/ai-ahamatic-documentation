...
## 1as. 🔶 OPEN 2026-08-20 — `04-human-in-the-loop-protocol.md` §5's closed trigger vocabulary has no AI-model-continuity row

**Found by H70 while designing the unresolved-deprecation blocking check, and correctly routed rather than forced onto the nearest existing row.** The spec's §5 trigger vocabulary is a **closed** thirteen-row set; an unresolved AI-model-continuity condition matches none of them. All three near-misses were resolved against their owning documents and **each fails on its owner's own definition**:

- the **third-party-vendor-risk** row's condition is one of the five re-assessment triggers the AI-governance spec explicitly states deprecation is *not*;
- the **backward-compatibility / managed-deprecation** row governs the platform's *own* announced deprecation path under INV-09 — a different subject reached by the same English word;
- the **invariant-breach** row is excluded by the spec's own statement that model-continuity is a category the enumerated invariants do not name.

**Routed to a `T##` per `PROCESS.md` §12.2** — an ADR may bind a specification document only where the obligation already exists there, and here it does not, which makes this a specification gap rather than a binding a design document may assert. **Deliberately not forced onto the nearest row: `BACKLOG.md` §1ap records what that interim step cost last time.**

**⚠ Nothing is left non-functional by leaving this open, and that is verified rather than assumed.** H70's blocking check reads the tracked model-dependency record directly and does not route through §5's vocabulary, so the mechanism works today. This is a completeness gap in the trigger vocabulary, not a broken mechanism — do not escalate it as the latter.
...
| ✅ **D-k** | **RESOLVED 2026-08-19 — `DECISIONS.md` D-55 (lead). Confirmed: already discharged.** `01-technology-stack-design.md` §2.6's criteria 11–13 (human-verifiability, commercial acceptability, AI-training-corpus quality) **are** the 2026-07-29 standup material — confirmed directly by the lead after an exhaustive repo-wide search for a transcript or summary file found none exists. The tracker line (`TICKET.md` line 188) simply never got updated once the material was recorded downstream — the same stale-entry pattern D-n turned out to be. No `docs/` amendment owed; the citation's one-day date imprecision (crediting a "2026-07-30 Standup" TICKET.md section rather than 2026-07-29) is optional cleanup, not a defect. | `TICKET.md` line 188 corrected as a tracker edit | ✅ Done |
...
**Full coverage audit run 2026-08-14 — `COVERAGE-AUDIT-2026-08-14.md` at repo root.** Every area below is routed from that report rather than re-derived; read it before scoping any remaining item, since it states the verified coverage/gap and a reasoned verdict for all seventeen.

- ✅ **Tenant provisioning — CLOSED 2026-08-19 by T85 (`DECISIONS.md` D-60, lead).** Thin, mechanical admission stated: no self-service/billing/KYC process modeled; admission requires no criteria beyond the Tenant admission role's own existing authority; admission establishes the tenant owner's first `platform.platform_users` record and top-authority scope in one act. **Design-counterpart scoped 2026-08-20 — a real gap, not a citation fix.** `02-tenant-isolation-and-access-control-design.md` §6.1 designs builder admission (adding a person to an *already-existing* tenant) as one INSERT into `tenant_<id>.tenant_users`, but explicitly does not cover the tenant *owner's* own first such row — and `04-scalability-availability-and-performance-design.md` §5.3's onboarding mechanism covers schema/role/grant creation only, never the tenant owner's own identity or binding row. Line 202 of the tenant-isolation document states plainly that tenant/application provisioning's own write is "presumed platform-core, DDL-capable administrative code," never actually designed anywhere. **Authorized H69** — a small extension composing with §6.1's existing mechanism (same one-INSERT shape, triggered by the Tenant-admission steward role's own admission act rather than an existing tenant owner's onward grant), never a new mechanism. **✅ DESIGN COUNTERPART CLOSED 2026-08-20 by H69, verified against the file:** new **§6.3** plus **ADR-065** in `02-tenant-isolation-and-access-control-design.md`; composition held exactly as scoped (ADR-061's shape consumed unchanged, only trigger/connection/precondition differing); the write runs over provisioning's own administrative connection, since §6.1's request-scoped reuse is **not constructible** for a steward holding no schema grant. **No new `event_type`** — `builder-admission` reused as a direct fit. **The scoping hypothesis this entry recorded was confirmed by the Executor, not merely inherited:** the gap was real. **Three residues routed rather than absorbed** — §1at (first-credential establishment, a genuine design gap) and §1au(a)–(b) (two owed citation amendments). **This entry is now fully closed at both layers.**
- ✅ **Platform SLA — RESOLVED 2026-08-19, `DECISIONS.md` D-56 (lead).** Out of scope — a commercial/legal artifact, not technical-specification content. No ticket.
...
- ✅ **Builder onboarding — fully resolved.** Narrow slice (adding a builder to an already-provisioned tenant): closed by **T79**. Training/certification half: **RESOLVED 2026-08-19, `DECISIONS.md` D-56 (lead)** — out of scope, no ticket.
- ✅ **Application lifecycle — RESOLVED, already closed before this session (correcting a 2026-08-19 error).** `COVERAGE-AUDIT-2026-08-14.md`'s finding (a PRD/journey inconsistency over whether C-15/C-17 cover a built application's own retirement) was accurate when written but stale by the time this entry was last touched. **Both halves were already closed 2026-08-18**: the journey text was corrected to the platform-only reading (`DECISIONS.md` D-40, **T80**), and the actual transition mechanism — who may trigger `suspended`/`offboarding`, under what preconditions — was designed independently (`DECISIONS.md` D-41, **H64**, `02-tenant-isolation-and-access-control-design.md` §6.2, ADR-063). A same-day decision (`DECISIONS.md` D-57) asked this question again without re-checking current state and was withdrawn once the staleness was caught mid-ticket-drafting, before any document was touched — see D-57's own entry for the record. No ticket owed.
- ✅ **Platform usage / acceptable-use policy — RESOLVED 2026-08-19, `DECISIONS.md` D-56 (lead).** Out of scope — a commercial/legal artifact, not technical-specification content. No ticket.
...
- ✅ **AI model governance and AI output-quality standards — BOTH CLOSED 2026-08-20 by T86 (`DECISIONS.md` D-58, lead).** New document `10-ai-model-governance-and-output-quality-standards.md` (Governance & Security, tenth document), propagated into `context-document-map.md`. Model governance operationalizes the invariants document's blocking-check rule for an unresolved model/model-version deprecation; output-quality is a deliberately non-blocking guardrail-metric-shaped floor, never a gate. No model, vendor, or agent framework named. **Design-counterpart scoped 2026-08-20 — a real gap, substantial enough to need its own investigation.** The new spec document itself states three times that it designs no selection mechanism, abstraction layer, measurement technique, or quality gate — every "how" question is explicitly deferred to Design phase. Neither `09-ai-assisted-builder-tooling-design.md` nor `05-ai-tooling-security-design.md` currently designs a model-selection criterion, a lock-in-mitigation abstraction, a deprecation-transition mechanism, or an output-quality tracking mechanism. **Authorized H70** — scoped as an investigation (document shape not pre-judged), mirroring T86's own method. **✅ DESIGN COUNTERPART CLOSED 2026-08-20 by H70, verified against the files:** shape determined as **one new document** — `11-ai-model-governance-and-output-quality-design.md`, the library's **45th** document and Layer 2's 11th — on four stated grounds argued in its own §2.1, because each candidate anchor's own words exclude the content and extending either would reverse a stated boundary. **ADR-066** minted; `08-audit-and-traceability-design.md` §4.5 amended (two `event_type` values, 66 → 68) under that section's own binding obligation. **The spec's §4/§5 asymmetry was honored, not flattened** — §4 is a blocking check composing onto existing gates (no new gate, no sixth kernel precondition, no new classification row); **§5 gets no mechanism at all, as a finding rather than a deferral**, no direct measurement being constructible. **One residue routed** — §1as (the missing HITL trigger row), which the blocking check does **not** depend on. **This entry is now fully closed at both layers.**
- ✅ **Bias / fairness — DECIDED 2026-08-19, `DECISIONS.md` D-59 (lead). Closed, no ticket.** Left to each client engagement, on the identical precedent `development-principles.md` already sets for a full security/compliance posture. No specification content added.
- ✅ **Pricing / monetization — Out of scope, decided by the audit itself (2026-08-14), no lead input needed.** A structural conclusion: the charter's own "what the platform IS" framing has no slot for commercialization, and the one document mentioning commercial scale frames it as outside the capability library by definition.
- ✅ **Go-to-market — Out of scope, decided by the audit itself (2026-08-14), no lead input needed.** Identical structural reasoning to pricing, reached independently.
- ✅ **Accessibility / internationalization — FULLY RESOLVED.** UI localization: settled, out of scope (`PROCESS.md` §5). Accessibility conformance: **DECIDED 2026-08-19, `DECISIONS.md` D-61 (lead). Closed, no ticket.** Stays an engineering choice (ADR-060's own stance), no compliance target committed.
...
- ✅ **Load testing — FULLY CLOSED 2026-08-20 by T87 (`DECISIONS.md` D-65, lead).** Detect-in-production stated explicitly in §4, citing the schema-count crossover point as the concrete, already-shipped instance. No pre-release load-testing regime added. **Design-counterpart checked 2026-08-20 — no ticket needed.** `03-testing-and-quality-gates-design.md` §5.4 already states the identical strategy in more analytical depth than the new spec paragraph (the "entirely" assignment to production monitoring, the reasoned-not-verified status of the waypoints) — it predates and already fully realizes what the spec now states as a "what." The design layer needs no new content; T87's spec addition is the missing statement of an already-designed, already-shipped practice.
