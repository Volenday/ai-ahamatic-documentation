...
- ✅ **Tenant provisioning — CLOSED 2026-08-19 by T85 (`DECISIONS.md` D-60, lead).** Thin, mechanical admission stated: no self-service/billing/KYC process modeled; admission requires no criteria beyond the Tenant admission role's own existing authority; admission establishes the tenant owner's first `platform.platform_users` record and top-authority scope in one act. **One item remains, not yet scoped:** a small design-counterpart follow-on to `02-tenant-isolation-and-access-control-design.md` — whether it needs new content or only a citation back to this obligation.
...
- ✅ **AI model governance and AI output-quality standards — BOTH CLOSED 2026-08-20 by T86 (`DECISIONS.md` D-58, lead).** New document `10-ai-model-governance-and-output-quality-standards.md` (Governance & Security, tenth document), propagated into `context-document-map.md`. Model governance operationalizes the invariants document's blocking-check rule for an unresolved model/model-version deprecation; output-quality is a deliberately non-blocking guardrail-metric-shaped floor, never a gate. No model, vendor, or agent framework named. **One item remains, not yet scoped:** a design-counterpart follow-on determining whether `09-ai-assisted-builder-tooling-design.md` and/or `05-ai-tooling-security-design.md` need amendment or only a citation back to this new document.
