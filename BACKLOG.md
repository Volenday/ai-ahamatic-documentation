...
## 1ar. 🔶 OPEN 2026-08-19 — no governing document owns the human-notification channel the entire Human-in-the-Loop mechanism depends on
...
**Not yet routed to a ticket.** This requires a scope decision first, on the same shape `BACKLOG.md` §1ad's own docket already uses: is a notification mechanism platform-core (every deployment gets one), or left to deployment-time configuration (an operational concern outside the design library's own scope, the way `01-environment-and-configuration-design.md` treats some infrastructure choices)? Whichever way it resolves determines whether this needs a `T##` spec gap, an `H##` design ticket, or closes as out of scope for this library — the same three-way shape T78/T81's own region-outage and non-IT-continuity findings were routed through. Surfaced here for the lead or team to scope, not pre-judged.
