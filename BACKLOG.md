...
- ✅ **Tenant provisioning — DECIDED 2026-08-19, `DECISIONS.md` D-60 (lead), ticket pending.** Thin — mechanical admission only, no self-service/billing/KYC. **Authorizes a `T##`** extending `03-access-control-and-tenancy-model.md` with the admission trigger and credential-issuance process, then a small design-counterpart follow-on. Sequencing note unchanged: pairs naturally with builder onboarding's narrow half (already closed by T79) in the same document family.
...
- 🔶 **Application lifecycle — DECIDED 2026-08-19, `DECISIONS.md` D-57 (lead), ticket pending.** C-15/C-17 do cover a built application's own retirement, resolving the internal inconsistency between the PRD's own capability definitions and `05-user-journeys.md`'s "Safe evolution" journey. **Authorizes a `T##`** to amend C-15/C-17's own PRD text, then a follow-on Layer 4 `H##` to design the retirement-transition mechanism. Sequencing note unchanged: `platform.applications` and `platform.tenants` share the identical `active`/`suspended`/`offboarding` enum with an equally undesigned transition mechanism in both cases — one design pattern may close both once scoped.
...
- ✅ **AI model governance — DECIDED 2026-08-19, `DECISIONS.md` D-58 (lead), ticket pending.** Yes — the library gains model-selection/vendor-lock-in-mitigation/deprecation-handling guidance despite the domain-neutrality tension. **Authorizes a `T##`**, scoped together with AI output-quality standards as one investigation (below).
- ✅ **AI output-quality standards — DECIDED 2026-08-19, `DECISIONS.md` D-58 (lead), ticket pending.** Yes — the library gains a correctness/quality standard for what AI-assisted tooling produces, beyond the existing binary provenance gate. **Scoped together with AI model governance** as one `T##` investigation, per the audit's own "two ends of one undesigned pipe" finding — not the same question, but likely to converge on one document or a closely-linked pair.
