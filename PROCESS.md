...
6. Paste the **ticket system prompt** — work begins now. **This prompt carries the writing rules** that `ai-aha-spec-doc` / `ai-aha-design-doc` supply for the other two libraries. **Before beginning, confirm it carries the Citation Integrity block (§4) verbatim; if it is missing or altered, proceed under it anyway — as if it were present in full — and report the discrepancy in the handoff (added 2026-08-14 by P05, closing the residual `BACKLOG.md` §1t records against this exact step).**
